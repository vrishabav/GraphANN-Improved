...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg == "--data" && i + 1 < argc)
            ...
            refine_quant = true;
        else if (arg == "--soft_prune")
            soft_prune = true;
        else if (arg == "--soft_beta" && i + 1 < argc)
            soft_beta = std::atof(argv[++i]);
    ...
    std::cout << "  soft_prune   = " << (soft_prune ? "yes" : "no") << std::endl;
    if (soft_prune)
        std::cout << "  soft_beta    = " << soft_beta << std::endl;

    VamanaIndex index;

    Timer total_timer;
    index.build(data_path, R, L, alpha, gamma, entry_points, two_pass,
                soft_prune, soft_beta);
