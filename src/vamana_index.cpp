...
// ============================================================================
// Soft Diversity Pruning
// ============================================================================
// An alternative to the hard binary alpha-RNG rule. Instead of rejecting a
// candidate outright when it violates the alpha condition, we compute a
// "diversity penalty" based on the maximum directional overlap between the
// candidate and already-selected neighbors, then re-rank all candidates by
// a combined score:
//
//   score(c) = dist(node, c) * (1 + beta * max_cos_overlap(c, selected))
//
// where max_cos_overlap is the maximum cosine similarity between the
// direction (node -> c) and any direction (node -> selected_neighbor).
// A high overlap means c is "in the same direction" as an already-selected
// neighbor, so it gets penalized.
//
// After scoring, we select the top-R candidates by score (lower = better).
// This is a soft version: no candidate is hard-rejected, but redundant
// directions are penalized. The alpha parameter still acts as a hard cap
// on the maximum distance ratio (same as robust_prune), ensuring we don't
// keep very far candidates.
//
// beta=0.0: degenerates to pure nearest-neighbor selection (no diversity)
// beta=0.5: moderate diversity penalty (default)
// beta=2.0: strong diversity penalty (similar to hard alpha-RNG)

void VamanaIndex::robust_prune_soft(uint32_t node, std::vector<Candidate> &candidates,
                                     float alpha, uint32_t R, float beta)
{
    // Remove self
    candidates.erase(
        std::remove_if(candidates.begin(), candidates.end(),
    ...
    const float *node_vec = get_vector(node);

    // Precompute difference vectors (node -> candidate), normalized
    // We'll use these to compute directional overlap
    uint32_t ncands = (uint32_t)candidates.size();
    std::vector<std::vector<float>> diff_vecs(ncands, std::vector<float>(dim_));
    std::vector<float> diff_norms(ncands, 0.0f);

    for (uint32_t i = 0; i < ncands; i++) {
        const float *cv = get_vector(candidates[i].second);
        float norm_sq = 0.0f;
        for (uint32_t d = 0; d < dim_; d++) {
            float diff = cv[d] - node_vec[d];
            diff_vecs[i][d] = diff;
            norm_sq += diff * diff;
        }
        diff_norms[i] = (norm_sq > 1e-9f) ? std::sqrt(norm_sq) : 1.0f;
    }

    // Greedy selection with soft scoring
    std::vector<uint32_t> selected;
    selected.reserve(R);
    std::vector<bool> used(ncands, false);

    // Also apply the hard alpha cap: skip candidates that are too far
    // relative to the closest candidate (same spirit as alpha-RNG)
    float closest_dist = candidates[0].first;

    for (uint32_t iter = 0; iter < R; iter++) {
        // Find the candidate with the lowest score among unused ones
        ...
        for (uint32_t i = 0; i < ncands; i++) {
            ...
            float dist_to_node = candidates[i].first;

            // Hard alpha cap: skip if too far relative to closest
            // (prevents keeping very distant candidates)
            if (dist_to_node > alpha * alpha * closest_dist)
                continue;

            // Compute max directional overlap with already-selected neighbors
            float max_overlap = 0.0f;
            for (uint32_t s : selected) {
                // Find index of s in candidates
                // (linear scan is fine since R is small, typically <= 64)
                for (uint32_t j = 0; j < ncands; j++) {
                    if (candidates[j].second == s) {
                        // Cosine similarity between direction to i and direction to j
                        float dot = 0.0f;
                        for (uint32_t d = 0; d < dim_; d++)
                            dot += diff_vecs[i][d] * diff_vecs[j][d];
                        float cos_sim = dot / (diff_norms[i] * diff_norms[j] + 1e-9f);
                        // Clamp to [0, 1]: negative overlap (opposite directions) = no penalty
            ...
            // Combined score: distance penalized by directional overlap
            float score = dist_to_node * (1.0f + beta * max_overlap);
        ...
        if (best_idx == UINT32_MAX) break;  // no more valid candidates
...
void VamanaIndex::build(const std::string &data_path, uint32_t R, uint32_t L,
                        ...
                        bool two_pass, bool soft_prune, float soft_beta)
{
    ...
    auto run_pass = [&](float pass_alpha, int pass_num)
    {
        ...
        std::cout << "Pass " << pass_num << " (alpha=" << pass_alpha
                  ...
                  << ", prune=" << (soft_prune ? "soft" : "hard") << ")..." << std::endl;
...
#pragma omp parallel for schedule(dynamic, 64)
        for (int64_t idx = 0; idx < (int64_t)npts_; idx++)
        {
            ...
            // Choose pruning strategy: hard alpha-RNG or soft angular diversity
            if (soft_prune)
                robust_prune_soft(point, prune_candidates, pass_alpha, R, soft_beta);
            else
            ...
            for (uint32_t nbr : graph_[point])
            {
                ...
                if (graph_[nbr].size() > gamma_R)
                {
                    ...
                    if (soft_prune)
                        robust_prune_soft(nbr, nbr_candidates, pass_alpha, R, soft_beta);
                    else
