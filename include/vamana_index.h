...
public:
  ...
  void build(const std::string &data_path, uint32_t R, uint32_t L,
             ...
             bool two_pass = true, bool soft_prune = false,
             float soft_beta = 0.5f);
...
private:
  ...
  // Soft diversity pruning: scores candidates by a combined distance +
  // angular diversity penalty. Instead of the hard binary alpha-RNG rule,
  // each candidate is scored as:
  //   score(c) = dist(node, c) * (1 + beta * max_overlap(c, selected))
  // where max_overlap measures how much c's direction overlaps with already-
  // selected neighbors (via normalized dot product of difference vectors).
  // Candidates are re-ranked by score and top-R are selected.
  // beta controls the diversity weight (0 = pure distance, higher = more diverse).
  void robust_prune_soft(uint32_t node, std::vector<Candidate> &candidates,
                         float alpha, uint32_t R, float beta = 0.5f);
